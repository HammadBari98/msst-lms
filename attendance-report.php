...
<head>
    ...
    <style>
        ...
        .att-empty { color: #ccc; }
        .summary-cell { pointer-events: none; } /* Disable hover on totals */
        
        /* Freeze the first two columns (Student Info) while scrolling horizontally */
        ...
        /* DataTables overrides to play nice with sticky positioning */
...
<script>
    $(document).ready(function() {
        // Initialize DataTables for Pagination (Keeps Sticky CSS intact by avoiding scrollX wrap)
        if ($('#reportTable').length > 0) {
            $('#reportTable').DataTable({
                "pageLength": 15,
                "lengthMenu": [10, 15, 25, 50, 100],
                "ordering": false, // Sorting disabled so date columns don't scramble
                "language": {
    ...
    function openEditModal(studentId, studentName, dateStr, currentStatus) {
        ...
        else document.getElementById('opt_present').checked = true; // Default if empty
        
        new bootstrap.Modal(document.getElementById('quickEditModal')).show();
    }

    function printReport() {
        // We use vanilla JS to clone the table directly to avoid DataTables wrappers
        ...
        // Reconstruct a clean table for print
