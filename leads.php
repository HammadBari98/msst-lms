...
try {
    ...
    try {
        $pdo->query("SELECT admission_status FROM admissions LIMIT 1");
    } catch (PDOException $e) {
        $pdo->exec("ALTER TABLE admissions ADD COLUMN admission_status VARCHAR(20) DEFAULT 'Pending'");
    }
    ...
    <div class="content-wrapper">
        <div class="container-fluid">
            ...
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 class="h3 mb-0 text-gray-800">College Leads Management</h1>
            </div>
                            ...
                            <tbody>
                                <?php if (!empty($leads)): ?>
                                    ...
                                    ?>
                                        <tr>
                                            <td><?= $sl++ ?></td>
                                            <td class="font-mono"><?= htmlspecialchars($lead['form_no'] ?? 'N/A') ?></td>
                                            <td class="fw-bold"><?= htmlspecialchars($lead['full_name'] ?? ($lead['name'] ?? 'N/A')) ?></td>
                                            <td><span class="badge bg-primary"><?= htmlspecialchars($lead['program'] ?? 'N/A') ?></span></td>
                                            <td><?= htmlspecialchars($lead['mobile'] ?? 'N/A') ?></td>
                                            ...
                                            <td>
                                                <div class="btn-group shadow-sm">
                                                    ...
                                                    <button class="btn btn-success btn-sm" title="Approve & Enroll" onclick="approveLead(<?= $lead['id'] ?>, 'college', '<?= htmlspecialchars(addslashes($lead['full_name'] ?? $lead['name'] ?? 'Student')) ?>')">
                                    ...
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <tr><td colspan="8" class="text-center py-4 text-muted"><i class="fas fa-inbox fa-2x mb-2 d-block"></i> No leads found.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
