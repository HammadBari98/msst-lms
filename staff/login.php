    ...
    if (empty($staff_id_string) || empty($password)) {
        ...
        if ($user && $user['role_name'] == 'Staff' && password_verify($password, $user['password'])) {
            if ($user['status'] == 'Inactive') {
                ...
                $_SESSION['staff_user_db_id'] = $user['id'];
...
<head>
    ...
    <style>
        :root { --primary: #906833; --secondary: #6c757d; --light-bg: #f8f9fc; }
        ...
        .brand-icon img { background-color: white; border-radius: 50%; width: 80px; height: 80px; padding: 5px; margin-bottom: 1rem; }
    </style>
