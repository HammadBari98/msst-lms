...
if ($current_user_db_id > 0) {
    try {
        // A. Fetch Overall Statistics (All-Time) - Unaffected by Off Days
        ...
        $raw_monthly = $stmt_monthly->fetchAll(PDO::FETCH_ASSOC);

        // C. Fetch Official Off Days
        ...
        $off_days = $stmt_offs->fetchAll(PDO::FETCH_ASSOC);

        // Merge Data for UI Timeline
        $timeline_data = [];
        
        foreach ($off_days as $od) {
            $timeline_data[$od['off_date']] = [
                'attendance_date' => $od['off_date'],
                'status' => 'Off Day - ' . $od['title'],
                'is_off' => true
            ];
        }
        foreach ($raw_monthly as $rec) {
            // Standard records overwrite off days in the highly unlikely event a teacher forced a mark
            if(!isset($timeline_data[$rec['attendance_date']])) {
                $rec['is_off'] = false;
                $timeline_data[$rec['attendance_date']] = $rec;
            }
        }
        
        krsort($timeline_data); // Sort Newest First
        $monthly_records = array_values($timeline_data);
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover align-middle mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr <?= $record['is_off'] ? 'class="bg-light"' : '' ?>>
